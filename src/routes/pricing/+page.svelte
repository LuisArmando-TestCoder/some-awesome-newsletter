...
<div id="smooth-wrapper" use:useSmoothPage>
  <div id="smooth-content">
    <div class="pricing">
      <div class="pricing__container">
        ...
        <div class="pricing__grid">
          {#if state?.content?.plans}
            {#each state.content.plans.filter((p) => !p.internalOnly && (p.id === "free" || p.id === state.interval)) as plan}
              <section
                class="pricing__plan pricing__plan--{plan.id}"
                class:current={plan.id === state.currentPlan}
                aria-labelledby="plan-name-{plan.id}"
              >
                <div class="pricing__plan-head">
                  <h2 id="plan-name-{plan.id}" class="pricing__plan-name">
                    {plan.name}
                  </h2>
                  <p class="pricing__plan-desc">{plan.tagline}</p>
                </div>
                <div class="pricing__price">
                  <span class="pricing__price-amount"
                    >${state.interval === "monthly"
                      ? plan.monthly
                      : plan.yearly}</span
                  >
                  <span class="pricing__per"
                    >/{state.interval === "monthly" ? "mo" : "yr"}</span
                  >
                </div>
                ...
                {#if plan.id === state.currentPlan}
                  <button class="pricing__cta current-plan-button" disabled
                    >This is your current plan</button
                  >
                {:else if state.content && state.content.plans.find((p) => p.id === state.currentPlan) && plan.tier < state.content.plans.find((p) => p.id === state.currentPlan)!.tier}
                  <a
                    ...
                    class="pricing__cta">Downgrade to {plan.name}</a
                  >
                {:else}
                  <a
                    href={`/api/checkout?products=${plan.productId}`}
                    class="pricing__cta">Upgrade to {plan.name}</a
                  >
                {/if}
              </section>
            {/each}
        ...
        <div class="pricing__contact">
          ...
          <a href={state?.content?.contact.href} class="pricing__contact-link"
            >{state?.content?.contact.cta}</a
          >
        </div>

        {#if state?.mockMode}
          <div class="pricing__banner--mock">Mock Mode Enabled</div>
        {/if}
      </div>
    </div>
    ...
    <style lang="scss">
      ...
      .pricing__toggle--annual .pricing__toggle-handle {
      ...
      .pricing__cta {
        ...
        transition:
          background 0.35s cubic-bezier(0.075, 0.82, 0.165, 1),
          box-shadow 0.5s;
      ...
      .pricing__table thead .pricing__table-cell {
        font-weight: 700;
      }
      .pricing__table-row + .pricing__table-row .pricing__table-cell {
      ...
      .pricing__table .is-yes::before {
        content: "✓";
        margin-right: var(--space-sm);
      }
      .pricing__table .is-no::before {
        content: "—";
        margin-right: var(--space-sm);
        opacity: 0.5;
      }
      ...
      .pricing__faq-item--open .pricing__faq-a {
        display: block;
      }
      ...
      /* States */
      .pricing__plan--disabled {
        opacity: 0.6;
        pointer-events: none;
      }
      .pricing__cta[aria-disabled="true"] {
        opacity: 0.6;
        pointer-events: none;
      }
