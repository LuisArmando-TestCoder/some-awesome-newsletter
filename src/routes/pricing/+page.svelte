...
<div id="smooth-wrapper" class="page-wrapper" use:useSmoothPage>
  <div id="smooth-content" class="page-content">
        ...
        <div class="pricing__grid">
          {#if state?.content?.plans}
            {#each state.content.plans.filter(p => p.id === 'free') as plan}
              <article class="pricing__card pricing__card--free">
                ...
                <ul class="pricing__features">
                  <li class="pricing__feature-item pricing__feature-item--primary">
                    <strong>100</strong> {$t.billing.users}
                  </li>
                  <li class="pricing__feature-item pricing__feature-item--primary">
                    <strong>1</strong> {$t.billing.newsSources}
                  </li>
            ...
            {#each PLANS_CONFIG as tier}
              {@const planData = state.content.plans.find(p => p.id === tier.id) || state.content.plans[0]}
              <article class="pricing__card" class:pricing__card--featured={tier.id === 'growth'}>
                {#if tier.id === 'growth'}
                  <div class="pricing__card-badge">{$t.pricing.mostPopular}</div>
                {/if}

                <div class="pricing__card-header">
                  <h2 class="pricing__card-name">{tier.id.toUpperCase()}</h2>
                ...
                <div class="pricing__price-block">
                  <span class="pricing__currency">$</span>
                  <span class="pricing__amount">{calculatePrice(tier.mo, state.interval)}</span>
                  <span class="pricing__period">/{state.interval === 'monthly' ? 'mo' : 'yr'}</span>
                </div>

                <ul class="pricing__features">
                  <li class="pricing__feature-item pricing__feature-item--primary">
                    <strong>{tier.users}</strong> {$t.billing.users}
                  </li>
                  <li class="pricing__feature-item pricing__feature-item--primary">
                    <strong>{tier.sources}</strong> {$t.billing.newsSources}
                  </li>
                ...
                <a href={`/api/checkout?products=${planData.productId}&interval=${state.interval}`} 
                   class="pricing__button" 
                   class:pricing__button--primary={tier.id === 'growth'}
                   class:pricing__button--dark={tier.id !== 'growth'}>
                   {$t.pricing.upgradeTo.replace('{planName}', tier.id)}
                </a>
              </article>
            {/each}
          {/if}
